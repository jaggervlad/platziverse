...
let AgentStub = null;
let db = null;
let sandbox = null; // Obtenemos un unico agente para poder hacer las pruebas

const single = Object.assign({}, agentFixtures.single); // Condicion  para obt a un agent por uuid
...
test.beforeEach(async () => {
  ...
  AgentStub.findById.withArgs(id).returns(Promise.resolve(agentFixtures.byId(id))); // Modelo findAll Stub

  AgentStub.finAll = sandbox.stub();
  AgentStub.finAll.withArgs().returns(Promise.resolve(agentFixtures.all));
  AgentStub.finAll.withArgs(connectedArgs).returns(Promise.resolve(agentFixtures.connected));
  AgentStub.finAll.withArgs(usernameArgs).returns(Promise.resolve(agentFixtures.platzi)); // Requerir los modelos con parametros por default
...
test.serial('Agent#findByUsername', async t => {
  ...
  t.is(agent.length, agentFixtures.platzi.length, 'agents should be the same amount');
  t.deepEqual(agent, agentFixtures.platzi, 'agents should be the same');
